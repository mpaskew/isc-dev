...
function display_day( $day, $monthView ) {
	...
	for ( $i = 0; $i < count( $monthView[$day] ); $i++ ) {
		...
		?>
        <?php if(tribe_get_organizer() == 'Illinois Science Council') { $organizerClass = "org_isc"; }
        else { $organizerClass = "org_unknown"; } ?>
		<div id='event_<?php echo $eventId; ?>' <?php post_class('tribe-events-event tribe-events-real-event ' .  $organizerClass) ?>>
